...
def lanczos_resampling(original_image, OH, OW, a=3):
    # Lanczos kernel
    L = lambda vec: np.sinc(vec) * np.sinc(vec/a)

    # Assuming original_image is H by W by C or H by W matrix, where C is the channel count
    H, W, *C = original_image.shape
    if C == []:
        original_image = original_image[..., np.newaxis]
    original_image = original_image.astype(np.float64) # H by W by C

    #< Resample image w.r.t. rows >
    # Choose indices to resample
    ...
    #</Resample image w.r.t. row >

    #< Resample image w.r.t. columns >
    # Choose indices to resample
...
def resample_image(file_path, source_dir, target_dir, short_side_size, quality=91):
    ...
    if os.path.exists(new_file_path) == True: return # Override it
...
def process_directory(source_dir, target_dir, short_side_size, max_jobs, quality=91):
    jpg_files = [os.path.join(dirpath, filename) for dirpath, dirnames, filenames in os.walk(source_dir)
                 for filename in filenames if filename.lower().endswith('.jpg')]
